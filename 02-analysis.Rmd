...
merged <- merged %>%
  ...
  select(treatment, `TotalEnrollment`, ENI, PercentBlack, 
         PercentSWD, PercentPoverty, borough, `% Chronically Absent`, `% Attendance`, borough)
...
$$
\begin{align}
ChronicAbsenteeism = \beta_0 +\beta_1Treatment + BoroughFixedEffects + \epsilon
\end{align}
$$
...
working_data$ps <- school_nearest$distance
  ...
  working_data$new_outcome_labelled <- ifelse(working_data$treatment == 1, "no SC", "SC")
  working_data$iptw <- ifelse(working_data$treatment == 1, 1/(working_data$ps),1/(1-working_data$ps))
  working_data$stable.iptw <-ifelse(working_data$treatment == 1,(mean(working_data$ps))/working_data$ps,mean(1-working_data$ps)/(1-working_data$ps))
  
  working_data_weighted <-svydesign(ids =~1, data=working_data,weights = working_data$iptw)
  school_covariates <-c("PercentBlack", "PercentSWD", "PercentPoverty", "ENI", "TotalEnrollment", "borough")
  
  ecls_table <-svyCreateTableOne(vars=school_covariates,strata="new_outcome_labelled",data=working_data_weighted,test=F)
  nearest_matched <- match.data(school_nearest)
library(broom)
## estimating treatment effects
lm(`% Chronically Absent` ~ treatment + borough, weights=working_data$iptw,data=working_data) %>%
 tidy() %>%
  kable(caption =  "Borough as Match Model, IPTW estimates of the ATE",
        col.names = c("Predictor", "B", "SE", "t", "p"),
        digits = c(0,2,3,2,3))
...
borough_vector <- c("Bronx", "Manhattan", "Staten Island", "Queens", "Brooklyn")
for (i in 1:length(borough_vector)){
borough_c <- as.character(borough_vector[i])
data <- merged %>%
  filter(borough == borough_c)
assign(paste0(borough_c, "data"), data)
school_nearest_borough <- matchit(treatment ~ PercentSWD + PercentPoverty + TotalEnrollment +
                          ENI, 
                          family = binomial(),
                          data = data,
                          method = "nearest",
                           ...
                           ratio = 2)
#create the matched set
nearest_matched2 <- match.data(school_nearest_borough)
assign(paste0(borough_c, "nearest"), school_nearest_borough)
assign(paste0(borough_c, "matched"), nearest_matched2)
}
...
#### Outcome Model: Manhattan

```{r echo=FALSE, message=FALSE, warning=FALSE}

working_data <- Manhattandata
#iptw
  working_data$ps <- Manhattannearest$distance
  #create IPTW weights - 0 is no SC, which is the treatment
  working_data$new_outcome_labelled <- ifelse(working_data$treatment == 1, "no SC", "SC")
  working_data$iptw <- ifelse(working_data$treatment == 1, 1/(working_data$ps),1/(1-working_data$ps))
  working_data$stable.iptw <-ifelse(working_data$treatment == 1,(mean(working_data$ps))/working_data$ps,mean(1-working_data$ps)/(1-working_data$ps))
...
library(broom)
## estimating treatment effects
lm(`% Chronically Absent` ~ treatment, weights=working_data$iptw,data=working_data) %>%
 tidy() %>%
  kable(caption =  "Within Manhattan Matched: IPTW estimates of the ATE",
        col.names = c("Predictor", "B", "SE", "t", "p"),
        digits = c(0,2,3,2,3))
...
working_data <-Bronxdata
#iptw
  working_data$ps <- Bronxnearest$distance
  #create IPTW weights - 0 is no SC, which is the treatment
  working_data$new_outcome_labelled <- ifelse(working_data$treatment == 1, "no SC", "SC")
  working_data$iptw <- ifelse(working_data$treatment == 1, 1/(working_data$ps),1/(1-working_data$ps))
  working_data$stable.iptw <-ifelse(working_data$treatment == 1,(mean(working_data$ps))/working_data$ps,mean(1-working_data$ps)/(1-working_data$ps))
...
lm(`% Chronically Absent` ~ treatment, weights=working_data$iptw,data=working_data) %>%
 tidy() %>%
  kable(caption =  "Within Bronx Matched: IPTW estimates of the ATE",
        col.names = c("Predictor", "B", "SE", "t", "p"),
        digits = c(0,2,3,2,3))

```

#### Outcome Model: Brooklyn

```{r echo=FALSE, message=FALSE, warning=FALSE}

working_data <- Brooklyndata
#iptw
  working_data$ps <- Brooklynnearest$distance
  #create IPTW weights - 0 is no SC, which is the treatment
  working_data$new_outcome_labelled <- ifelse(working_data$treatment == 1, "no SC", "SC")
  working_data$iptw <- ifelse(working_data$treatment == 1, 1/(working_data$ps),1/(1-working_data$ps))
  working_data$stable.iptw <-ifelse(working_data$treatment == 1,(mean(working_data$ps))/working_data$ps,mean(1-working_data$ps)/(1-working_data$ps))
...
lm(`% Chronically Absent` ~ treatment, weights=working_data$iptw,data=working_data) %>%
 tidy() %>%
  kable(caption =  "Within Brooklyn Matched: IPTW estimates of the ATE",
        col.names = c("Predictor", "B", "SE", "t", "p"),
        digits = c(0,2,3,2,3))

```

#### Outcome Model: Queens

```{r echo=FALSE, message=FALSE, warning=FALSE}

working_data <- Queensdata
#iptw
  working_data$ps <- Queensnearest$distance
  #create IPTW weights - 0 is no SC, which is the treatment
  working_data$new_outcome_labelled <- ifelse(working_data$treatment == 1, "no SC", "SC")
  working_data$iptw <- ifelse(working_data$treatment == 1, 1/(working_data$ps),1/(1-working_data$ps))
  working_data$stable.iptw <-ifelse(working_data$treatment == 1,(mean(working_data$ps))/working_data$ps,mean(1-working_data$ps)/(1-working_data$ps))
...
lm(`% Chronically Absent` ~ treatment, weights=working_data$iptw,data=working_data) %>%
 tidy() %>%
  kable(caption =  "Within Queens Matched: IPTW estimates of the ATE",
        col.names = c("Predictor", "B", "SE", "t", "p"),
        digits = c(0,2,3,2,3))
